 ...
 */
public class SearchListBottomDialogCommand extends Command {

    private final String TAG = SearchListBottomDialogCommand.class.getSimpleName();
    private Activity mActivity;
    private WaterCallBack mCommandCallBack;
    private static String mDialogCode = "0000";
    private static int mDialogResourceId;

    private static SearchListBottomDialogCommand mBaseDialogCommand;
    ...
    public static final String EXT_SEARCH_BEAN = "search_bean";

    //생성자
    public SearchListBottomDialogCommand(){

    }

    public SearchListBottomDialogCommand(Activity activity, WaterCallBack commandCallback){
        mActivity = activity;
        mCommandCallBack = commandCallback;
    }

    //싱클톤
    public static SearchListBottomDialogCommand getInstance(){
        if(mBaseDialogCommand == null){
            mBaseDialogCommand = new SearchListBottomDialogCommand();
        }
     ...
     */
    public SearchListBottomDialogCommand(Activity activity, String dialogCode, int resourceId, WaterCallBack commandCallback){
        mActivity = activity;
    ...
    public SearchListBottomDialogCommand setBaseDialogCommand(Activity activity, String dialogCode, int resourceId, WaterCallBack commandCallback){
        mActivity = activity;
        mDialogCode = dialogCode;
        mDialogResourceId = resourceId;
        mCommandCallBack = commandCallback;
    ...
    private void goBaseDialog(Activity activity, Object object, BaseBean baseData){
        ...
        final BottomDialogSearchList baseBottomDialog = new BottomDialogSearchList(activity, object);
        ...
        baseBottomDialog.setOnSelectListener(new WaterCallBack() {
            @Override
            public void callback(BaseBean baseData) {
                //선택시, 콜백
                Bundle bundle = new Bundle();
                bundle.putString(EXT_BTN_TYPE, BTN_TYPE.SELECT.name());
                bundle.putParcelable(EXT_SEARCH_BEAN, (SearchListBean)baseData.getObject());
